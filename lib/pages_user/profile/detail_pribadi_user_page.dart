...
class DetailPribadiUserPage extends StatefulWidget {
  const DetailPribadiUserPage({super.key, required this.profile, required this.idProfile, required this.listJurusan, required this.daftarUniv});

  final ProfileUserModel profile;
  final String idProfile;
  final List<JurusanModel> listJurusan;
  final List<String> daftarUniv;
...
class _DetailPribadiUserPageState extends State<DetailPribadiUserPage> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.listJurusan.length; i++) {
      daftarJurusan.add(widget.listJurusan[i].namaJurusan);
    }
    if (profile != null) {
  ...
  void getDataRational() async {
    try {
      ...
      for (int i = 0; i < selectedJurusan.length; i++) {
        for (int j = 0; j < widget.listJurusan.length; j++) {
          if (widget.listJurusan[j].namaJurusan == selectedJurusan[i]) {
            listJurusanPilihan.add(JurusanModel(namaJurusan: widget.listJurusan[j].namaJurusan, relevance: widget.listJurusan[j].relevance, value: 0));
          }
        }
      }
        ...
        if (isFound) {
          ...
          if (ganti) {
            await RationalizationUserService.add(
              RationalizationUserModel(jurusan: rationalUser.first.jurusan, created: DateTime.now(), userUID: userUid, idTryout: '', idUserTo: ''),
            );
          }
          ...
          await RationalizationUserService.add(RationalizationUserModel(jurusan: listJurusanPilihan, created: DateTime.now(), userUID: userUid, idTryout: '', idUserTo: ''));
        }
      } else {
